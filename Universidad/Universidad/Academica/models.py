...
class Carrera(models.Model):
	...
	duracion = models.PositiveSmallIntegerField(default=5,null=False)
	def __str__(self):
		txt = "{0} (Duración: {1} año(s))"
		return txt.format(self.nombre, self.duracion)

class Estudiante(models.Model):
	dni = models.CharField(max_length=8,null=False,primary_key=True)
	apellidoPaterno = models.CharField(max_length=35,null=False)
	apellidoMaterno = models.CharField(max_length=35,null=False)
...
class Curso(models.Model):
	codigo= models.CharField(max_length=6,null=False, primary_key=True)
